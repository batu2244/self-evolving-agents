...
| 7 | Bug `bug-mrzh0sj7-bb0e` (high) | Clicking a quick-reply chip fires **two identical** `POST /api/onboarding/chat` requests and renders a duplicate assistant reply | `post()` was called inside the `setMsgs` state updater; React 19 StrictMode double-invokes updaters, so the side effect ran twice. Replay's root-cause analysis named the exact file and mechanism | Moved the API call out of the updater — message list computed in the event handler, `setMsgs` gets a plain value, `post()` runs once | `frontend/src/modules/onboarding/components/Chat.tsx` |

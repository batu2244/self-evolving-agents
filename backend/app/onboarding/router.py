...
from app.onboarding.llm import llm_extract, stream_reply
...
async def _run_turn(body: ChatRequest) -> ChatResponse:
...
async def chat_stream(body: ChatRequest) -> StreamingResponse:
    """SSE variant: the structured turn is computed deterministically, then the
    concierge's reply text is generated by Claude and streamed token by token
    (`delta` events), followed by one `turn` event with the full payload."""
    result = await _run_turn(body)
    history = [m.model_dump() for m in body.messages]

    async def gen():
        parts: list[str] = []
        async for delta in stream_reply(history, result.reply):
            parts.append(delta)
            yield f"event: delta\ndata: {json.dumps({'text': delta})}\n\n"
        payload = result.model_dump(by_alias=True)
        payload["reply"] = "".join(parts).strip() or result.reply

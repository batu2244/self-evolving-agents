...
def derive_trend(closes: Sequence[float], equation: str | None = None) -> dict:
    """Historical analyst: trend slope + MA relationship + mean-reversion flag."""
    equation = equation or config.EQUATION_BY_AGENT["historical"]
    if len(closes) < 2:
        return {
            "equation": equation,
            "direction": 0.0,
    ...
    if equation == "trend_blend":
        # Trend is the core read; the MA relationship confirms or tempers it.
        direction = clamp(0.6 * slope_component + 0.4 * ma_component)
    elif equation == "slope_only":
        direction = slope_component
    elif equation == "ma_cross":
        direction = ma_component
    else:
        raise ValueError(f"unknown historical equation {equation!r}")
    ...
    bits.append(f"equation={equation}")
    if relationship != "unknown":
...
def derive_momentum(quote: dict, equation: str | None = None) -> dict:
    """Realtime analyst: price vs open / previous close, plus a volume anomaly check."""
    equation = equation or config.EQUATION_BY_AGENT["realtime"]
    ...
    if equation == "balanced_momentum":
        move = _mean([p[0] for p in parts])
    elif equation == "previous_close_only":
        move = vs_prev if prev_close else 0.0
    elif equation == "open_weighted":
        if open_price and prev_close:
            move = 0.7 * vs_open + 0.3 * vs_prev
        elif open_price:
            move = vs_open
        else:
            move = vs_prev
    else:
        raise ValueError(f"unknown realtime equation {equation!r}")
...
def derive_news(decision: dict, signal_score: dict, equation: str | None = None) -> dict:
    ...
    """
    equation = equation or config.EQUATION_BY_AGENT["news"]
    ...
    action_direction = 0.3 if action == "BUY" else (-0.3 if action == "SELL" else 0.0)
    if equation == "weighted_score":
        pass
    elif equation == "action_conviction_blend":
        direction = clamp(0.7 * direction + 0.3 * action_direction * conviction)
    elif equation == "article_count_fade":
        direction = clamp(direction * (0.5 + 0.5 * coverage))
    else:
        raise ValueError(f"unknown news equation {equation!r}")
    ...
    return {
        ...
        "rationale": (
            ...
            f"{scored} scored articles (equation={equation}, score {direction:+.3f})."
        ),
    }


def candidate_summary(result: dict) -> dict:
    """Compact, JSON-safe view of one candidate equation's read."""
    keep = (
        "equation",
        "direction",
        "confidence",
...
def evaluate_trend_candidates(closes: Sequence[float]) -> list[dict]:
    return [
        candidate_summary(derive_trend(closes, equation=equation))
        for equation in config.EQUATION_CHOICES["historical"]
    ]


def evaluate_momentum_candidates(quote: dict) -> list[dict]:
    return [
        candidate_summary(derive_momentum(quote, equation=equation))
        for equation in config.EQUATION_CHOICES["realtime"]
    ]


def evaluate_news_candidates(decision: dict, signal_score: dict) -> list[dict]:
    return [
        candidate_summary(derive_news(decision, signal_score, equation=equation))
        for equation in config.EQUATION_CHOICES["news"]
    ]
